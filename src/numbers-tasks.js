...
function roundToPowerOfTen(num, pow) {
  const len = 10 ** num.toString().length;
  const numPount = num / len;
  const numFixed = numPount.toFixed(num.toString().length - pow);
  return numFixed * len;
}
...
function getFibonacciNumber(index) {
  let pastValue = 0;
  let nowValue = index ? 1 : 0;
  for (let i = 1; i < index; i += 1) {
    [pastValue, nowValue] = [nowValue, nowValue + pastValue];
  }
  return nowValue;
}
...
function getSumOfDigits(num) {
  ...
  return arrayNum.reduce((a, b) => Number(a) + Number(b));
}
